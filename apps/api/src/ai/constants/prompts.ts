export const PROMPTS = {
  ...
  TOOL_CALL_SYSTEM: `
            ...
            [생존확률/창업성공 예측 도구 선택 - 매우 중요]
            사용자가 "살아남을 수 있을까?", "망할 확률", "생존 가능성", "버틸 수 있을까" 등 구체적인 생존 여부나 확률을 물어보면, 반드시 'predict_survival_rate' 도구를 사용하세요.
            단순 폐업률(통계)만 묻는게 아니라 '미래 예후'나 '가능성'을 묻는 질문에 이 도구를 최우선으로 선택하세요.
            - 예: "강남역 카페 살아남을까?" → predict_survival_rate
            - 예: "여기서 치킨집 하면 망할 확률은?" → predict_survival_rate
  ...
  ANALYZE_RESULTS_SYSTEM: `
    ...
    - "임대료", "수익", "창업비용", "얼마 벌어", "순수익", "손익분기", "본전" 키워드 → rent.calculate (임대료 및 수익/BEP 분석)
    - "생존", "살아남을까", "폐업률", "망할 확률", "얼마나 버텨" 키워드 → prediction.survival (생존확률 예측)
    ...
    [특수 규칙: 수익/비용 분석 결과 해석]
    - 'estimate_revenue_and_cost' 도구 결과의 'rentSource'가 '통계 추정'인 경우, 반드시 사용자에게 다음과 같이 제안하세요.
      "이 결과는 평균 임대료를 기준으로 계산되었습니다. 더 정확한 분석을 위해 **실제 부동산 매물**을 추천해 드릴까요?"
    - 사용자가 이에 동의하면 'real_estate.recommend' 도구를 사용할 수 있음을 암시하세요.
