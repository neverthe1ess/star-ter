...
export const fetchAiSummary = async (areaName: string, metrics: string) => {
  const response = await fetch(`${API_BASE_URL}/ai/summary`, {
    ...
    body: JSON.stringify({ areaName, metrics }),
  });
...
export const formatMetricsForAi = (
  ...
  const demoTotal = demographics?.reduce(
    (sum: number, d: any) =>
      sum + (Number(d.male) || 0) + (Number(d.female) || 0),
    0,
  );
  ...
  return `
    ...
    1. 주요 업종 (매출 상위):
    ${sectors?.map((s: any) => `- ${s.name}: ${s.value}`).join('\n') || '데이터 없음'}

    2. 업종 포화도:
    ${saturation?.map((s: any) => `- ${s.name}: ${s.value}`).join('\n') || '데이터 없음'}

    3. 매출 추이:
    ${growth?.map((g: any) => `- ${g.period}: ${g.amount}`).join('\n') || '데이터 없음'}
    ...
    5. 시간대별 유동인구:
    ${population?.map((p: any) => `- ${p.time}시: ${p.value}명`).join('\n') || '데이터 없음'}
  `;
};
