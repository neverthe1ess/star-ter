...
export function TrafficFilterBar({
  ...
  // 현재 연령대 라벨
  const currentAgeLabel = AGE_SLIDER_OPTIONS.find(opt => opt.value === ageFilter)?.label || '전체';
  const currentAgeIndex = AGE_SLIDER_OPTIONS.findIndex(opt => opt.value === ageFilter);
      ...
      {isOpen ? (
        <div className="w-full max-w-[440px] bg-white/40 backdrop-blur-2xl rounded-[40px] p-6 shadow-2xl border border-white/40 pointer-events-auto animate-spring-up ring-1 ring-black/5">
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-gray-900 font-bold text-xl leading-tight">유동인구 히트맵</p>
          ...
          <div className="bg-white/30 rounded-3xl p-4 border border-white/50 shadow-sm mb-4">
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="flex items-center gap-3">
                <span className="text-orange-500 font-bold text-xl">{hourLabel}</span>
                <button
                  onClick={onTogglePlay}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-xl font-bold text-sm transition-all ${
                    isPlaying
          ...
          <div className="bg-white/30 rounded-3xl p-4 border border-white/50 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-blue-900 font-bold text-lg">{currentAgeLabel}</span>
            </div>
        ...
        >
          ...
          <div className="flex flex-col items-start">
            <span className="text-gray-900 font-bold text-xl leading-none mb-1">{hourLabel}</span>
