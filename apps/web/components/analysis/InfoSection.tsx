    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          const message = errorData.message || res.statusText || '데이터를 불러오는데 실패했습니다.';
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50/30">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-64 space-y-4">
              ...
              <p className="text-gray-400 text-sm font-medium animate-pulse">상권 데이터를 분석하고 있습니다...</p>
            </div>
          ) : error ? (
              ...
              <div className="px-6 py-2 space-y-4">
                  ...
                  <div className="p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-gray-50 rounded-xl">
                        <p className="text-xs text-gray-400 font-bold mb-1 uppercase tracking-wider">주요 특징</p>
                        <p className="text-sm text-gray-700 leading-relaxed font-medium">{data.zoneOverview?.characteristics}</p>
                      </div>
                      <div className="p-4 bg-gray-50 rounded-xl">
                        <p className="text-xs text-gray-400 font-bold mb-1 uppercase tracking-wider">방문 동기</p>
                        <p className="text-sm text-gray-700 leading-relaxed font-medium">{data.zoneOverview?.visitMotivation}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'population' && (
                  <div className="animate-in zoom-in-95 duration-300">
                    <TimePopulationChart data={data} areaName={selectedArea?.name} />
                  </div>
                )}
                ...
                {(activeTab === 'industry' || activeTab === 'risk') && data.competitionAnalysis && (
                  <div className="p-6 bg-white rounded-2xl border border-gray-100 shadow-sm space-y-4">
                    ...
                    {data.competitionAnalysis.map((item, idx) => (
                      <div key={idx} className="p-4 bg-emerald-50/50 rounded-xl border border-emerald-100/50">
                        <p className="text-xs text-emerald-600 font-bold mb-1 uppercase tracking-wider">{item.category}</p>
                        <p className="text-sm text-gray-700 font-bold mb-1">{item.summary}</p>
                        <p className="text-[13px] text-gray-500 leading-relaxed">{item.implication}</p>
                      </div>
                    ))}
                  </div>
                )}

                {(activeTab === 'cost' || activeTab === 'risk') && data.conclusion && (
                  <div className="p-6 bg-white rounded-2xl border border-gray-100 shadow-sm space-y-4">
                    <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                    ...
                    <div className="space-y-3">
                      {data.conclusion.map((item, idx) => (
                        <div key={idx} className="flex gap-3 p-3 hover:bg-gray-50 rounded-lg transition-colors group">
                          ...
                          <div>
                            <p className="text-xs text-amber-600 font-bold mb-0.5">{item.category}</p>
