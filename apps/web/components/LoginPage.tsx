...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleGuestLogin = () => {
      // Placeholder for guest login
      console.log("Guest login clicked");
      // Could potentially just call onLogin() with a mock user if requested, but plan said placeholder.
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-[0_0_15px_rgba(0,0,0,0.1)] w-full max-w-5xl flex overflow-hidden min-h-[540px] max-h-[540px]">
        ...
        <div className="w-1/2 p-16 flex flex-col justify-center border-r border-gray-100">
          <div className="flex justify-center mb-12">
             <div className="flex items-center">
                <span className="text-8xl font-bold text-[#1e2b5e]">Starter</span>
             </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4 w-full max-w-sm mx-auto">
            <div>
              <Input
                type="text" // Changed to text to match "사용자 아이디" (User ID) generic feel, or keep email if backend requires email
                placeholder="사용자 아이디"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                required
                className="h-12 text-base bg-white border-gray-300 focus:!border-blue-900 focus:!ring-blue-900 rounded-md"
              />
            </div>
            ...
            <Button
              ...
              className="w-full bg-[#1e2b5e] hover:bg-[#151f45] text-white h-12 text-base rounded-md font-medium transition-colors"
            >
        ...
        {/* Right Column: Social / Guest Login */}
        <div className="w-1/2 p-16 flex flex-col justify-center items-center bg-white">
          <div className="w-full max-w-xs space-y-4 text-center">
            <h3 className="text-gray-900 font-[500] text-[14px]">Sign In with</h3>
